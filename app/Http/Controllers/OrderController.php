...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $orders = Order::where('post_type', 'shop_order')->where('post_status','publish')->where('post_author', $user->ID)->with(['items', 'items.meta', 'meta'])->get();
        if($orders){
    ...
    {
        $order = Order::with(['items', 'items.meta', 'meta'])->where('post_status','publish')->findOrFail($id);
